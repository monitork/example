...
class Gap extends LeafRenderObjectWidget {
  ...
  @override
  void updateRenderObject(BuildContext context, covariant RenderGap renderObject) => renderObject.gap = gap;
}
...
class RenderGap extends RenderBox {
  ...
  void performLayout() {
    ...
    assert(direction != null, 'gap should not be used outside of a Flex (Row, Column)');
    if (direction == null) {
      size = Size(0, gap);
    } else {
      size = Size(direction == Axis.horizontal ? gap : 0, direction == Axis.vertical ? gap : 0);
    }
  }
}

...
/// Get singleton logger by `Logger()`
///
/// ```dart
/// final logger = Logger();
/// ```
final console = Logger()..mode = LoggerMode.log;
...
class Logger {
  ...
  /// if you wrap this Logger by your own logger.
    ...
    switch (mode) {
      case LoggerMode.log:
        developer.log(
          ...
          name: 'Flutter',
          time: info.time,
          stackTrace: includeCallerInfo && level >= stackTraceLevel ? StackTrace.current : null,
        );
        break;
      case LoggerMode.print:
        // ignore: avoid_print
  ...
  Frame? _getCallerFrame() {
    ...
    // Expensive
    const baseLevel = 3;
    final frames = Trace.current(baseLevel + _callerInfoFrameLevelOffset).frames;
